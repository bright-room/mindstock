...
class HouseholdRpcServiceE2eTest : FunSpec({
    ...
    test("findOf returns the household the actor owns") {
        e2eTest {
            ...
            val rpc = authenticatedRpcClient(asUser = owner, path = "household")
    ...
    test("findOf returns null when the actor has no household") {
        e2eTest {
            val orphan = seedUser()
            val rpc = authenticatedRpcClient(asUser = orphan, path = "household")
                .withService<HouseholdRpcService>()
    ...
    test("invite adds a new MEMBER, observable when that user calls findOf") {
        e2eTest {
            ...
            val ownerRpc = authenticatedRpcClient(asUser = owner, path = "household")
                .withService<HouseholdRpcService>()

            ownerRpc.invite(household.id, invitee.id, HouseholdMemberRole.MEMBER)

            val inviteeRpc = authenticatedRpcClient(asUser = invitee, path = "household")
                .withService<HouseholdRpcService>()
    ...
    test("revoke removes the member, observable as null findOf for that user") {
        e2eTest {
            ...
            val memberRpc = authenticatedRpcClient(asUser = member, path = "household")
                .withService<HouseholdRpcService>()
