...
@OptIn(ExperimentalUuidApi::class)
class RequireUnregisteredUserPluginTest :
    FunSpec({
        ...
        test("session 有り + userId null → 200") {
            testApplication {
                application {
        ...
        test("session 有り + userId 非 null → 409") {
            testApplication {
                application {
                ...
                client.get("/guarded").status shouldBe HttpStatusCode.Conflict
            }
        }
