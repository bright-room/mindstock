...
| 日時ピッカー（バックデート） | ~~**外す**~~ → **P6-4b で復活**（補充/消費に DatePick）。訂正=数量+理由のまま | ~~`occurredAt` を受け取らずサーバ時刻で確定（P3 決定）~~ → P6-4b で覆した。`replenish`/`consume` はクライアント指定 `occurredAt`（バックデート可）、`correct` のみサーバ now 維持。詳細 `docs/superpowers/specs/2026-06-08-occurred-at-backdate-design.md` |
