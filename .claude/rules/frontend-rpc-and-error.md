...
- **ViewModel のエラーハンドリングは `FailureHandler`(`core/ui/FailureHandler.kt`)に集約済み**。各 VM はコンストラクタで受けた `reauth`/`toast` から `private val failure = FailureHandler(reauth, toast)` を 1 行持つ。用途に応じて 2 種を使い分ける:
  - `failure.onMutationFailure(error)` — 書込失敗。`Unauthorized` → `reauth.request()`、それ以外 → `toast.show(errorText(error))`(画面状態は変えない)。
  - `failure.onMutationFailure(error, conflictText: StringResource?)` — Conflict かつ `conflictText` 指定時は専用文言でトースト。それ以外は上記と同じ。
  - `failure.onLoadFailure(error)` — 読込失敗。`Unauthorized` → `reauth.request()` のみ。**トーストは出さない**(`UiState.Error` に文言を出す呼び出し側と二重表示しない)。`UiState.Error(errorText(error))` への遷移は呼び出し側のままに残す。
