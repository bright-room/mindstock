...
class StockForecastTest {
    ...
    fun A_定常はトレーリング窓のレートで予測する() {
        ...
        stock.forecast(EvaluatedTime(asOf)) shouldBe ConsumptionForecast.DaysRemaining(15)
    }
    ...
    fun B_直近窓に消費が無ければ全履歴平均にfallbackする() {
        ...
        stock.forecast(EvaluatedTime(asOf)) shouldBe ConsumptionForecast.DaysRemaining(40)
    }
    ...
    fun C_履歴の浅い新商品はspanベースで予測する() {
        ...
        stock.forecast(EvaluatedTime(asOf)) shouldBe ConsumptionForecast.DaysRemaining(7)
    }
    ...
    fun D_消費実績が無ければUnknown() {
        ...
        stock.forecast(EvaluatedTime(asOf)) shouldBe ConsumptionForecast.Unknown
    }
    ...
    fun F_訂正後の実効消費量でレートを算出する() {
        ...
        stock.forecast(EvaluatedTime(asOf)) shouldBe ConsumptionForecast.DaysRemaining(180)
    }
    ...
    fun 窓境界_ちょうど60日前の消費はトレーリングに含む() {
        ...
        stock.forecast(EvaluatedTime(asOf)) shouldBe ConsumptionForecast.DaysRemaining(540)
    }
    ...
    fun トレーリング窓は窓外の古い消費を除外する() {
        ...
        stock.forecast(EvaluatedTime(asOf)) shouldBe ConsumptionForecast.DaysRemaining(360)
    }
    ...
    fun span1日クランプで0除算しない() {
        ...
        stock.currentQuantity()() shouldBe 3
        stock.forecast(EvaluatedTime(asOf)) shouldBe ConsumptionForecast.DaysRemaining(2)
    }
}
